...
搜索栏同时承担「附加留言」与「URL 输入」两职：

- **URL 前缀自动识别**：`http://` `https://` `file://` `x-devonthink-item://`
- **解绑必选文件**：作为需要文件参数的技能，输入 URL 后无需再到 Finder/DEVONthink/VS Code 选中文件
- **URL + 文件并行**：可同时选中本地文件 + 输入 URL，都作为参数传给技能
- **URL 后追加备注**：`https://example.com/x.pdf 帮我总结` → URL 与「帮我总结」都会被技能接收
