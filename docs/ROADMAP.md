...
- **✅ 扩展入口与执行稳定性修复**: 修复 Raycast 命令图标、类型检查和后台执行转义问题
- **🔄 持续优化中**: 用户体验和稳定性持续改进
...
| **阶段五：体验优化**        | 提升用户体验和稳定性               | 5/9 完成   | 🟡 进行中   |
...
| 任务队列与并发控制 | 新增队列模块，支持配置最大并发数，超限自动排队，队列持久化到磁盘                           | 🔴 高  | ✅   | 2026-05-01 |
| 移除 5 分钟超时    | 移除 CLI 执行的超时限制，避免误杀长任务，改为依赖进程自然退出                              | 🟡 中  | ✅   | 2026-05-01 |
| 扩展入口与执行稳定性修复 | 修复 Raycast 命令图标、manifest checkbox 字段、后台执行转义、PID 取消和状态误判问题 | 🔴 高 | ✅ | 2026-05-20 |
...
- **任务队列与并发控制**: 新增队列功能，支持配置最大并发数（默认 5），超出自动排队
  - 新增 `src/utils/taskQueue.ts` 模块，基于 JSON 文件持久化队列
  - 队列在窗口关闭/重开后自动恢复，24 小时超时清理过期任务
  - 技能列表显示队列位置和取消排队操作
  - 状态页面新增"排队中"分区
- **移除 5 分钟超时**: 移除 `executeClaudeCommand` 的 5 分钟超时限制，避免误杀长任务
- **性能优化**: devonthink 异步化、skills 缓存、runningCount 缓存
- **文档更新**: 将 CLAUDE.md 纳入版本控制，更新重载说明为 `npm run dev` 自动重载
